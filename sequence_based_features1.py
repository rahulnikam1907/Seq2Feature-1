...
#    file1 = pd.read_csv('./data/prop_49_list.csv')
    ...
    wild = mutation[0]
    wrt = pd.DataFrame()
    wrt['Actual'] = (df1[mut]-df1[wild])
#    wrt['Normalized'] = (df_1[mut]-df_1[wild])
#    out = pd.concat([file1,wrt],axis =1)
    return(wrt.transpose())
def prop_49_norml(seq ,mutation):
#    df1 = pd.read_csv('./data/49_properties_numerical_Values.csv', sep =',' )
    df_1 = pd.read_csv('./data/49_properties_normalizedValues.csv', sep =',' )
...
#                    print("calculating... amino acid properties")
                    ...
                    pf_norml = prop_49_norml(str(seq) , str(mutation))
                    newDF_49 = newDF_49.append(pf, ignore_index=True)
                    newDF_49_norml = newDF_49_norml.append(pf_norml,ignore_index = True)
...
#""")
    ...
    newDF1_norml = pd.concat([file1,newDF_49_norml],axis =0)
    
    new = newDF1.transpose()
    new_norml = newDF1.transpose()
...
#    print(col1)
    newDF1.columns = col1
    newDF1_norml.columns = col1
    newDF1_norml['Class'] = class1
#            new.columns = cols
#        print"%s"% (new.to_html(columns= cols,index = False))
    newDF1[1:].round(3).to_csv("./out_protein/AAP_out_numerical.csv",header = True , index = False,index_label = False)
    newDF1_norml[1:].round(3).to_csv("./out_protein/AAP_out_normalized.csv",header = True , index = False,index_label = False)
...
#    newDF_neg.to_csv("./out_protein/neighbour.csv",index_label = False,index = False)
    cmd = "paste -d, "+"./out_protein/AAP_out_numerical.csv"+" ./out_protein/SUB_out.csv"+" ./out_protein/out_contact_potential.csv"+ " > ./output.csv"
    os.system(cmd)
